...
- Package manager: pnpm 8.x (monorepo)
...
- **Phase 1** (Weeks 1-2): Foundation — monorepo, DB, auth, tests.
...
- All prompts stored in code repository under `packages/prompts/` with versioned filenames.
...
│   └── llm-abstraction/       # LLM interface + impls
├── infra/
│   ├── terraform/             # IaC
...
pnpm install
cp .env.example .env
# Fill in .env with GROQ_API_KEY, GEMINI_API_KEY, STRIPE_SECRET_KEY, etc.
...
- LLM provider abstraction layer (packages/llm-abstraction)
- Evaluation prompt templates (packages/prompts)
...
- Admin dashboard (apps/api/src/routes/admin)
