...
export default function OpinionPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/opinions/${params.id}/opinion`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  return (
    <div className="max-w-3xl mx-auto mt-10 p-6 border rounded">
      ...
      <a href={`/api/opinions/${params.id}/opinion/pdf`} className="mt-4 inline-block bg-primary text-white rounded py-2 px-4">Download PDF</a>
    </div>
  );
}
